...
export default class {
  ...
  handleEditTicket(e, bill, bills) {
    ...
    console.log("l'event s'effectuer mec")
  ...
  handleShowTickets(e, bills, index) {
    ...
    bills.forEach(bill => {
      ...
      if ($element) {
        const $event = $._data($($element).get(0), "events")
        console.log($event)
        if (!$event) {
          $($element).on("click",(e) => this.handleEditTicket(e, bill, bills))
        }
      }})
